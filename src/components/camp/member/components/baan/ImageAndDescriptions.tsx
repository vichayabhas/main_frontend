...
export default function ImageAndDescriptions({
  ...
  const isHaveNongAllow =
    imageAndDescriptionContainers.filter((data) => data.mode == "nong").length >
    0;
  const chain = React.useState(
    getDefaultLockInit({ token, mode, bypass: !isHaveNongAllow })
  );
  const name =
  ...
  React.useEffect(() => {
    updateSocket.listen(
      role == "pee"
        ? setImageAndDescriptionContainers
        : (data) =>
            setImageAndDescriptionContainers(
              data.filter((v) => v.mode == "nong")
            )
    );
  ...
  return (
    <div className="w-[100%] flex flex-col items-center pt-20 space-y-10">
      <AllInOneLock token={token} mode={mode} bypass={isHaveNongAllow}>
        {selectIndex == null ? (
          <Select
            variant="standard"
            name="location"
          ...
          >
            {imageAndDescriptionContainers
              .filter((v) => {
                ...
                if (role == "pee" && mode == "pee") {
                  return true;
                }
                if (v.mode == "pee" && role == "nong") {
                  return false;
                }
                switch (v.types) {
                  case "boy":
              ...
              .map((v, i) => (
                <AllInOneLock link={chain} bypass={v.mode == "nong"} key={i}>
                  <MenuItem onClick={() => setSelectIndex(i)}>
                    {v.name}
                  </MenuItem>
                </AllInOneLock>
              ))}
